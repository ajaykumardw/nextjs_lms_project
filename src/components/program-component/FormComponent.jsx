...
const FormComponent = ({
  ...
  const handleFetchCertificate = useCallback(async () => {
    try {
      const response = await fetch(
        `${API_URL}/company/certificate/data`,
        {
          method: 'GET',
          headers: {
      ...
      console.error(error)
      toast.error('Something went wrong while fetching certificates')
    }
  }, [API_URL, token])
  ...
  const schema = object({
    ...
    description: pipe(
      ...
      maxLength(
        1000,
        'Description can be of max 1000 length'
      )
    ),
  ...
  useEffect(() => {
    if (editData) {
      ...
      setValue(
        'live_session_type',
        editData?.live_session_type || ''
      )

      setCheckCertificate(
        editData?.checkCertificate || false
      )

      setSelectedCertificateId(
        editData?.certificateId || null
      )
    }
  }, [editData, setValue])
      ...
      rejectedFiles.forEach(file => {
        file.errors.forEach(error => {
          ...
          switch (error.code) {
            ...
            default:
              msg =
                'There was an issue with the uploaded file.'
          }
  ...
  const onSubmit = async value => {
    ...
    if (
      ...
      toast.error(
        'Please select a certificate or uncheck the certificate option',
        {
          autoClose: 1000
        }
      )
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(
          getLocalizedUrl(backURL, locale)
        )
      } else {
        toast.error(
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      ...
                      {createData?.live_session
                        ?.length > 0 &&
                        createData.live_session.map(
                          item => (
                            <MenuItem
                              key={item._id}
                              value={item._id}
                            ...
                            >
                              ...
                              <Box
                                sx={{
                                  ...
                                  height:
                                    '100%',
                                  backgroundImage: `url(${ASSET_URL}/frames/${item.backgroundImage})`,
                                  backgroundSize:
                                    'cover',
                                  backgroundPosition:
                                    'center'
                              ...
                              >
                                ...
                                >
                                  {item?.logoURL && (
                                    <img
                                      src={`${ASSET_URL}/company_logo/${item.logoURL}`}
                                      alt='Logo'
                                      width={
                                        40
                                      }
